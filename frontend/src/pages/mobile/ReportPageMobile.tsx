...
function renderPolishedText(text: string): React.ReactNode[] {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        if (match[2] && match[3]) {
            parts.push(
                <span key={key++} className="m-polish-change">
                    <span className="m-deleted">{match[2]}</span>
                    <span className="m-added">{match[3]}</span>
                </span>
            );
        } else if (match[4]) {
            parts.push(<span key={key++} className="m-deleted">{match[4]}</span>);
        } else if (match[5]) {
            parts.push(<span key={key++} className="m-added">{match[5]}</span>);
        }
...
interface ReportPageMobileProps {
    result: UserAnswer;
    question: Question | null;
    onBack: () => void;
}

export default function ReportPageMobile({ result, question, onBack }: ReportPageMobileProps) {
    ...
    return (
        <div className="report-mobile">
            ...
            <div className="m-report-header">
                <button className="m-nav-back" onClick={onBack}>← 返回</button>
                    ...
                    <div className="m-tab-pane animate-fade-in">
                        <div className="m-card">
                            ...
                            <div className="m-polish-content">
                                {renderPolishedText(gradingResult.ai_feedback?.polished_with_marks || gradingResult.ai_feedback?.polished_version || '暂无润色数据')}
                            </div>
                        </div>
                    </div>
                )}
            ...
            <div className="m-fixed-footer">
                <button className="m-footer-btn secondary" onClick={onBack}>继续练习</button>
