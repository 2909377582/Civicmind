...
const GradingLoadingOverlay: React.FC<GradingLoadingOverlayProps> = ({
    ...
    // Simulation Timer Logic - 在 pending 和 processing 状态都启动动画
    useEffect(() => {
        ...
        const animate = () => {
            ...
            // Real backend progress override (if backend sends > simulated)
            // But keep it smooth
            if (status.progress > simulatedProgress) {
                // If backend is significantly ahead, jump (safeguard)
