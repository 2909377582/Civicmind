...
import GradingLoadingOverlay from '../components/GradingLoadingOverlay'

interface AnswerPageProps {
    question: Question
    exam?: Exam  // 可选的试卷信息（用于显示材料）
    onSubmit: (result: any) => void
    onBack: () => void
}

function AnswerPage({ question, exam, onSubmit, onBack }: AnswerPageProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            pollGradingStatus(response.answer_id, (result) => {
                // 构造完整的 UserAnswer 对象传给报告页
                // result 从 polling 返回的是 GradingResult (api.ts getStatus 定义)
                // 或者是 UserAnswer (如果 hook 做了处理)。
                // 为了保险，我们这里手动构造 UserAnswer
                const gradingResult = result as GradingResult;

                const fullUserAnswer: UserAnswer = {
                    id: response.answer_id,
                    question_id: question.id,
                    user_id: 'current-user', // 占位
                    content: answerContent,
                    image_url: answerMode === 'image' ? imageUrl || undefined : undefined,
                    word_count: answerContent.length,
